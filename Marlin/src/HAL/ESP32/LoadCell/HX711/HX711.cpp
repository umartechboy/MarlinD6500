...
bool HX711_ADC::getTareAsyncStatus()
{
	return asyncTareFlag || doTare;
}
void HX711_ADC::tareAsync()
{
	asyncTareFlag = true;
	tareRdy = 0;
	doTare = 1;
	tareTimes = 0;
	tareTimeoutFlag = 0;
	tareTimeout = millis() + tareTimeOut;
}
void HX711_ADC::	tareAsyncLoop()
{
	if (tareRdy != 2)
	{
		tareRdy = update();
...
float HX711_ADC::getData() // return fresh data from the moving average data set
{
	...
	if (isnan(calFactor) || isinf(calFactor) || calFactor == 0)
		calFactor = 1;
...
uint8_t HX711_ADC::conversion24bit()  //read 24 bit data and start the next conversion
{
	...
	{ //read 24 bit data + set gain and start next conversion
		...
		long start = micros();
		if (SCK_DELAY) delayMicroseconds(SCK_DELAY); // could be required for faster mcu's, set value in config.h
		digitalWrite(sckPin, 1);
		if (SCK_DELAY) delayMicroseconds(SCK_DELAY); // could be required for faster mcu's, set value in config.h
		digitalWrite(sckPin, 0); // need to go v fast on PCFs
		if (i < (24))
		{
	...
	// 		if (exampleDataCount <= 0)
	// 			lastData = data; // change the ref this this for the next time, accepting this data
	// 		else
	// 			data = lastData; // use previous data
	// 	}
	// 	else
	// 	{
	// 		lastData = data;	
	// 		exampleDataCount++;
	// 		if (exampleDataCount > 15)
	// 			exampleDataCount = 15;
	// 	}
	// }
	// Serial.print(exampleDataCount);
	// Serial.print(", ");
	// Serial.print(data, 2);
	// Serial.print(", ");
	// Serial.print(data);
	...
	//Serial.println("return conversion24bit()");
	return 0;
}
